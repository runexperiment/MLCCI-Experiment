...
# 决策树
def decision_tree(train_sample, train_target, ver_dict, pro_name,res_path,model_path,sum):
...
# 准备数据集和训练集
def machine_learning(root,data,error_pro_ver,res_path,model_path):
    # 获取程序路径
    res = Tool_io.create_data_folder(root, data)
    for index in res:
        # 程序名称
        pro_name = os.path.basename(index)
        if pro_name == 'Chart' or pro_name == 'Time' or pro_name == 'Closure' or pro_name == 'Lang' or pro_name == 'Math' or pro_name == 'Mockito' or pro_name == 'Collections':
            continue
        # 测试用例
        print(pro_name)
        ver_dict = {}
...
if __name__ =="__main__":
    ...
    machine_learning(root,data,error_pro_ver,res_path,model_path)
