...
def base_info(root, data, error_pro_ver, res_path):
    ...
    for pros in all:
        if pros != '/home/tianshuaihua/base_dataset/Math':
            continue
        for ver in all[pros]:
            print(ver)
...
def SBFL_location(aefList,aepList,anfList,anpList,covMatrix,tf,tp,ccList,WPCC):
    ...
    for statement_index in range(statement_num):
            ...
            ds_zreo = cal_dstar(tf, tp, aef, aep, anf, anp, 3)
            sus_ds_zero[statement_index] = ds_zreo
    ...
    #     sus_ds_three = sorted(sus_ds_three.items(), key=lambda d: d[1], reverse=True)
    # except:
    #     return {"ds":sus_ds_zero, "ds_c":sus_ds_one,'ds_r':sus_ds_two,'ds_error':sus_ds_three}

    formulaSus = {"ds": sus_ds_zero,"ds_c":sus_ds_one,'ds_r':sus_ds_two,'ds_e':sus_ds_three}
